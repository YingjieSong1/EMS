...
subject_ids=df[{args.set_name}].values.tolist()
subject_ids=[str(i[0]).rjust(3,'0') for i in subject_ids]
feature_dict = np.load(f'{args.feature_dict}', allow_pickle=True).item()

val_dataset = TestLoader(set_ids=subject_ids,feature_dict=feature_dict,args=args)
val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=args.no_workers)
val_model(model, val_loader, device, args)
